#!/usr/bin/env python3
"""
Liquidity Scalping Backtest for Solana Meme Coins
...
class LiquidityScalper:
    ...
    def detect_sweep_signals(self, candles: List[Candle]) -> List[Candle]:
        ...
        1. Price wicks below support (1-3% fakeout)
        2. Closes back above support within 2 candles
        """
        ...
        for i in range(20, len(candles) - 5):
            ...
            # Valid sweep: 1-5% below support (wider range for meme coins)
    ...
    def run_backtest(self, coins: Dict[str, str] = None, days_back: int = 14) -> Dict:
        ...
        print("=" * 70)
        print("LIQUIDITY SCALPING BACKTEST - Solana Meme Coins")
        print("=" * 70)
        ...
        for symbol, address in coins.items():
            ...
            if trades:
                ...
                avg_profit = mean([t['pnl_pct'] for t in trades]) * 100
                ...
                print(f"  ✅ Trades: {len(trades)} | Win Rate: {win_rate:.1f}% | Avg: {avg_profit:.4f}%")
            else:
...
def main():
    print("\n🚀 Liquidity Scalping Backtest\n")
    
    scalper = LiquidityScalper(BIRDEYE_API_KEY)
    results = scalper.run_backtest(MEME_COINS, days_back=14)
    
    # Save results
    with open("liquidity_scalper_results.json", 'w') as f:
        json.dump(results, f, indent=2)
    
    print("\n💾 Results saved to liquidity_scalper_results.json")
