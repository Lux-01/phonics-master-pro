#!/usr/bin/env python3
"""
Liquidity Scalping Backtest - Pump.fun Meme Coins
Strategy: Range-trading/mean reversion based on liquidity sweeps
Testing fresh/newer meme coins vs established ones
"""
...
# 7 Pump.fun meme coins (fresh/newer tokens)
PUMP_COINS = {
    "PUMP1": "F5tfztTnE4sYsMhZT5KrFpWvHmYSfJZoRjCuxKPbpump",
    "PUMP2": "3bzeQJqDPfQQbxJ6VmtMkXyyQEJrj9HS1SMvHR1Hpump",
    "PUMP3": "BzyKa1FGjs2EUpu3GGDibY4xdygn5evAiRboKmETpump",
    "PUMP4": "7fj85y28pKMndm4So66Szkb5GMGfLHFEkwsZdDx2pump",
    "PUMP5": "6ydMmFRaNt4AHBjgvSogbBvcxnTrX3QzUe2AEzVppump",
    "PUMP6": "HaUXAdAWWUkb2WTmh8z1TdaEmZkYjnQnmC6tHUdTAHvi",
    "PUMP7": "32CdQdBUxbCsLy5AUHWmyidfwhgGUr9N573NBUrDpump",
}
...
class LiquidityScalper:
    ...
    def run_backtest(self, coins: Dict[str, str] = None, days_back: int = 14) -> Dict:
        ...
        print("=" * 70)
        print("LIQUIDITY SCALPING BACKTEST - Pump.fun Meme Coins")
        print("=" * 70)
        print(f"Timeframe: 15m | Lookback: {days_back} days")
        print(f"Testing: 7 fresh pump.fun launched tokens")
        print("-" * 70)
        ...
        for symbol, address in coins.items():
            print(f"\n🔍 Testing {symbol} ({address[:20]}...)")
            
            candles = self.fetch_ohlcv(address, timeframe="15m", days_back=days_back)
            
            if not candles or len(candles) < 50:
                print(f"  ❌ Insufficient data - only {len(candles) if candles else 0} candles")
                unusable_tokens.append({
                    'symbol': symbol,
                    'address': address,
        ...
        results = {
            ...
            'token_data_quality': {
                ...
                'unusable_tokens': unusable_tokens,
                'usable_count': len(usable_tokens),
                'unusable_count': len(unusable_tokens)
            },
            'overall_metrics': {
            ...
            'comparison_to_mew': {
                'mew_win_rate': 80.0,  # From previous backtest
                'pump_coins_avg_win_rate': round(win_rate, 2) if all_trades else 0,
                'comparison': 'better' if win_rate > 80 else 'worse' if all_trades else 'no_data',
                'difference': round(win_rate - 80, 2) if all_trades else None
            }
        }
        
        print("\n" + "=" * 70)
        print("BACKTEST RESULTS SUMMARY - PUMP.FUN COINS")
        print("=" * 70)
        print(f"Tokens with usable data: {len(usable_tokens)}/7")
        print(f"Tokens without data: {len(unusable_tokens)}/7")
        if unusable_tokens:
            for t in unusable_tokens:
        ...
        print("-" * 70)
        print("COMPARISON TO MEW (Established Coin Winner):")
        print(f"  MEW Win Rate: 80.0%")
        print(f"  Pump Coins Avg Win Rate: {win_rate:.2f}%")
        if all_trades:
            diff = win_rate - 80.0
            print(f"  Difference: {diff:+.2f}% ({'better' if diff > 0 else 'worse'} than MEW)")
        print("=" * 70)
...
def main():
    ...
    results = scalper.run_backtest(PUMP_COINS, days_back=14)
    
    # Save results
    with open("liquidity_scalper_pump_results.json", 'w') as f:
        json.dump(results, f, indent=2)
    
    print("\n💾 Results saved to liquidity_scalper_pump_results.json")
