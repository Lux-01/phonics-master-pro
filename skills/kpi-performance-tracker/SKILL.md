...
description: Track key performance indicators across all systems. Automated reports, alerting, and trend analysis. Now includes comprehensive income tracking, MRR calculation, and ROI per skill analysis.
tier: 3
type: analytics
status: active
---
...
Track performance metrics across skills, services, and business outcomes.

## Capabilities

- **Skill metrics:** Success rates, execution times
...
```bash
python3 kpi_tracker.py --collect

python3 kpi_tracker.py --dashboard

python3 kpi_tracker.py --alert-config alerts.json

# Income tracking (NEW)
python3 kpi_tracker.py --income-dashboard

python3 kpi_tracker.py --mrr

python3 kpi_tracker.py --skill-roi
```

---

# 💰 BUSINESS KPIS - INCOME TRACKING

**Track, analyze, and grow your income.**

The Business KPIs module monitors every revenue stream, calculates true MRR (Monthly Recurring Revenue), and provides actionable insights to increase profitability per skill.
...
│   ├── system_metrics.json     # System health
│   └── user_metrics.json       # User analytics
├── business/
│   ├── income/
│   │   ├── streams.json        # All income sources
│   │   ├── transactions/       # Monthly transaction files
│   │   ├── mrr.json            # MRR calculation
│   │   └── goals.json          # Targets
│   └── roi/
│       ├── skill_roi.json      # ROI per skill
│       └── time_costs.json     # Time investment
├── reports/
│   ├── monthly/                # Monthly dashboards
│   ├── quarterly/              # Quarterly summaries
│   └── annual/                 # Year-end reports
├── alerts/
│   └── thresholds.json         # Alert configurations
└── config.json
```
...
KPI Performance Tracker +
  ...
  All skills contribute to performance metrics
```
...
**Track your KPIs. Optimize everything.** 📊
