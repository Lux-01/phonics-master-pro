#!/usr/bin/env python3
"""
Pre-Query Memory System
Automatically check memory before responding to user queries
"""
import os
import json
...
class PreQueryMemory:
        ...
        try:
            ...
            for category, keywords in api_keywords.items():
                if any(k in query for k in keywords):
                    # Look for relevant section in TOOLS.md
    ...
    def _check_memory_for_keywords(self, query: str, max_items: int) -> List[Dict]:
        ...
        # Determine which topic this is about
        matched_topic = None
        ...
        try:
            ...
            for filename in memory_files[-5:]:  # Last 5 days
    ...
    def _check_recent_decisions(self, query: str) -> List[str]:
        ...
        # Check if query relates to recent decisions
        decision_keywords = {
    ...
    def _generate_suggestions(self, query: str, context: Dict) -> List[str]:
        ...
        # If API keys mentioned but not in context
        if any(k in query for k in ['api', 'key', 'token', 'credential']):
            if not context['api_keys']:
                suggestions.append("I should check TOOLS.md for API keys")
...
gather_context = lambda query: get_pre_query().gather_context(query)
