// birdeye_trader.js - Birdeye API Trading Assistant
...
class BirdeyeTrader {
  ...
  async buy(tokenMint, solAmount) {
    ...
    if (!price) {
      console.log('❌ Could not get token price');
      return null;
    }
    ...
    console.log(`   Token Price: $${price.price.toFixed(10)} (${price.source})`);
    ...
    console.log('');
    console.log('   ⚠️  To execute this trade:');
    console.log('   1. Open https://axiom.trade');
    console.log('   2. Connect Phantom');
    console.log('   3. Enter token: ' + tokenMint);
    console.log('   4. Buy amount: ' + solAmount + ' SOL worth');
    console.log('');
  ...
  async sell(tokenMint, percentage = 100) {
    ...
    console.log(`   Selling: ${sellAmount.toFixed(4)} tokens (${percentage}%)`);
    console.log(`   Token price: $${price?.price?.toFixed(10) || 'N/A'} (${price?.source || 'unknown'})`);
    console.log(`   Estimated SOL: ~${solValue.toFixed(4)} SOL`);
    console.log('');
    console.log('   ⚠️  To execute this trade:');
    console.log('   1. Open https://axiom.trade');
    console.log('   2. Connect Phantom');
    console.log('   3. Select token: ' + tokenMint);
    console.log('   4. Sell amount: ' + percentage + '%');
    console.log('');
    
    return { balance, sellAmount, solValue, price: price?.price };
  }
}
...
async function main() {
  ...
  if (cmd === 'price' && params[0]) {
    const trader = new BirdeyeTrader({ publicKey: { toString: () => 'dummy' } });
    ...
    const trader = new BirdeyeTrader(wallet.getKeypair());
    ...
    console.log('Birdeye Trader - Price & Trading Assistant');
    console.log('');
    console.log('Commands:');
    console.log('  price <mint>                    - Check token price');
    console.log('  balance                          - Check wallet balance');
    console.log('  buy <mint> <amount>             - Simulate buy with calculations');
    console.log('  sell <mint> [percentage]         - Simulate sell with calculations');
    console.log('');
    console.log('Examples:');
    console.log('  node birdeye_trader.js price DezXAZ8z7PnrnRJjz3wXBoRgixCa6xjnB7YaB1pPB263');
    console.log('  node birdeye_trader.js balance');
    console.log('  node birdeye_trader.js buy DezXAZ8z7PnrnRJjz3wXBoRgixCa6xjnB7YaB1pPB263 0.001');
  }
}
