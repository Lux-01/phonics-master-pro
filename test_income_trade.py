#!/usr/bin/env python3
"""
🚀 BUY AND SELL TEST - INCOME Token
...
def main():
    print("="*70)
    print("🚀 BUY AND SELL TEST - INCOME TOKEN")
    print("="*70)
    print(f"Token: {TOKEN_SYMBOL}")
    print(f"Address: {TOKEN}")
    ...
    if buy_success and sell_success:
        ...
        print(f"   Token: {TOKEN_SYMBOL}")
        print(f"   Buy TX: {buy_result.get('transaction_signature', 'N/A')[:30]}...")
        print(f"   Sell TX: See above")
        print(f"   Amount: {tokens_received} {TOKEN_SYMBOL}")
        print(f"\n🎉 Both buy and sell executed successfully!")
    elif buy_success:
        print("⚠️ Buy succeeded but sell failed")
        print(f"   You now hold {tokens_received} {TOKEN_SYMBOL}")
        print(f"   Buy TX: {buy_result.get('transaction_signature', 'N/A')}")
        print(f"\n💡 You can sell manually via Jupiter:")
